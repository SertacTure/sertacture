(()=>{
  ...
  const addStyles=()=>{
    ...
    s.textContent=`
      .clips-v9{margin-top:42px;padding-top:6px}
      ...
      .clip-v9-meta strong{font-size:16px}
      .clip-v9-meta span{font:500 12px/1.3 'JetBrains Mono',monospace;color:var(--muted);letter-spacing:.04em}
  ...
  const build=()=>{
    ...
    if(nav&&!nav.querySelector('a[href="#klip"]')){
      ...
      a.href='#klip';
      a.textContent='Klip';
      const y=nav.querySelector('a[href="#yayin"]');
      nav.insertBefore(a,y||null);
    }

    Promise.all(Array.from({length:CHUNKS},(_,i)=>
      fetch(`./assets/clip_${String(i).padStart(2,'0')}.txt`,{cache:'force-cache'})
      ...
      const l=sec.querySelector('.clip-v9-loading');
      if(l)l.textContent='Klip yüklenemedi.';
    });
  };
